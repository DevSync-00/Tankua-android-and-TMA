          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <div className="space-y-8">
              ...
              <form
                ...
                className="flex max-w-xl items-center gap-2 rounded-2xl border border-[#FFB800]/25 bg-white p-2 shadow-[0_16px_45px_rgba(74,58,42,0.12)]"
              >
                <label className="flex min-h-12 flex-1 items-center gap-3 px-3">
                  <Search className="h-5 w-5 shrink-0 text-[#C48900]" />
                  <span className="sr-only">Search destinations and experiences</span>
                  <input
                    value={searchQuery}
                    onChange={(event) => setSearchQuery(event.target.value)}
                    placeholder="Search places, cities, or experiences"
                    className="w-full bg-transparent text-[#4A3A2A] outline-none placeholder:text-[#4A3A2A]/45"
                  />
                </label>
                <Button type="submit" className="min-h-12 bg-[#FFB800] px-5 font-semibold text-white hover:bg-[#E5A500]">
                  Search
                </Button>
              </form>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {tourCategories.map((category) => (
              <Link key={category.id} href={`/tours?category=${category.slug}`} className="block rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FFB800] focus-visible:ring-offset-2">
              <Card hoverable className="h-full p-6 group cursor-pointer natural-card">
